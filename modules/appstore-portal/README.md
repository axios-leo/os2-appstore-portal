...
`src/impl/artifact_receiver.hpp` 提供 `ReceivingStore`。软件工厂与商店不共享文件系统时，
通过管理总线完成三阶段上传：

1. `ArtifactUploadPrepare`：提交 `artifact_id`、`version`、`sha256`、`total_bytes` 和可选
   `sbom_ref`，获得 `upload_id` 与 `chunk_bytes`；
2. `ArtifactUploadChunk`：按严格递增的 `offset` 发送 `data_hex`；二进制使用十六进制编码，
   单块原始数据不得超过协商值；
3. `ArtifactUploadCommit`：完整性、大小和摘要复核通过后，复用 `ArtifactPublish` 的格式、
   签名、不可覆盖入库及 `(artifact_id, version)` 索引流程。

调用方中断时可发送 `ArtifactUploadAbort` 主动释放会话。会话超时会被回收，进程重启后
`.uploads` 中无法恢复的临时文件也会自动清理。签名证书和签名值仍由既有受控信任通道
配置，不混入制品字节通道。
...
- `store.upload_chunk_bytes`：分块原始字节上限，缺省 64 KiB，硬上限 1 MiB；
- `store.max_upload_sessions`：同时打开的上传会话上限，缺省 32；
- `store.upload_session_ttl_ms`：无活动会话超时，缺省 15 分钟；
- `store.gc_orphans_on_start`：启动时删除崩溃残留临时文件和无索引制品，缺省 `false`。

同一 `artifact_id+version+sha256` 重试为幂等成功；暂存文件缺失、摘要不一致、超限、
路径分量非法或仓内同版本内容不同均拒绝登记。
