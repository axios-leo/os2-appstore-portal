...
// 增量 SHA-256：调用方可以分块 update，避免为了计算摘要把整个制品读入内存。
class Sha256Stream {
 public:
  ...
  std::array<std::uint8_t, 32> finalize() {
    ...
    const std::uint64_t bitlen = total_bytes_ * 8;
    ...
    for (int i = 7; i >= 0; --i)
      buffer_[buffered_++] = static_cast<std::uint8_t>((bitlen >> (i * 8)) & 0xff);
    transform(buffer_.data());
    for (int i = 0; i < 8; ++i) {
      digest_[i * 4] = static_cast<std::uint8_t>(state_[i] >> 24);
      digest_[i * 4 + 1] = static_cast<std::uint8_t>(state_[i] >> 16);
      digest_[i * 4 + 2] = static_cast<std::uint8_t>(state_[i] >> 8);
